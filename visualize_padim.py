"""
visualize_padim.py — Generate visualizations for PaDiM only.
Outputs to results/predictions/PaDiM_predictions.png plus updated combined ROC.
"""
...
#Helpers (same look as visualize.py) 
...
def plot_sample_predictions(records, model_name, save_dir, n_samples=6, seed=42):
    ...
    for row, r in enumerate(samples):
        ...
        axes[row][2].imshow(contoured); axes[row][2].axis("off")
...
def plot_combined_roc(all_records, save_path):
    """Plot ROC curves for ALL three models together."""
    plt.figure(figsize=(8, 6.5))
    colors = {"PatchCore": "steelblue",
              "EfficientAD": "tomato",
              "PaDiM":      "seagreen"}
    ...
    for model_name, records in all_records.items():
        ...
        roc_auc = auc(fpr, tpr)
        plt.plot(fpr, tpr, color=colors.get(model_name, "black"), lw=2.5,
                 label=f"{model_name}  (AUROC = {roc_auc:.4f})")

    plt.plot([0, 1], [0, 1], "k--", lw=1.2, label="Random Classifier")
    plt.xlabel("False Positive Rate", fontsize=12)
    plt.ylabel("True Positive Rate", fontsize=12)
    plt.title("ROC Curves — Vial Anomaly Detection (3 Models)",
    ...
    plt.close()
    print(f"[Viz] Combined ROC → {save_path}")


def plot_combined_score_dist(all_records, save_path):
    n = len(all_records)
    ...
    for ax, (model_name, records) in zip(axes, all_records.items()):
        ...
        ax.hist(normal,  bins=30, alpha=0.6, color="steelblue",
                label="Normal",  density=True)
        ax.hist(anomaly, bins=30, alpha=0.6, color="tomato",
    ...
    fig.suptitle("Anomaly Score Distributions (All Models)",
                 fontsize=14, fontweight="bold")
    plt.tight_layout()
    plt.savefig(save_path, dpi=150)
    plt.close()
    print(f"[Viz] Combined score dist → {save_path}")
...
def main():
    ...
    #  PaDiM (always loaded if present) 
    padim_ckpt = os.path.join(args.checkpoints, "padim.pkl")
    if os.path.exists(padim_ckpt):
        print("\n[PaDiM] Loading...")
        m = PaDiM(device=str(device), img_size=args.img_size)
        m.load(padim_ckpt)
        all_records["PaDiM"] = collect_predictions(m, test_loader, device)
        plot_sample_predictions(
            all_records["PaDiM"], "PaDiM",
    ...
    # PatchCore (optional, for combined plots)
    pc_ckpt = os.path.join(args.checkpoints, "patchcore.pkl")
    if os.path.exists(pc_ckpt):
        print("\n[PatchCore] Loading for combined plots...")
        m = PatchCore(device=str(device), img_size=args.img_size)
        m.load(pc_ckpt)
        all_records["PatchCore"] = collect_predictions(m, test_loader, device)

    #  EfficientAD (optional) 
    ead_ckpt = os.path.join(args.checkpoints, "efficientad.pt")
    if os.path.exists(ead_ckpt):
        print("\n[EfficientAD] Loading for combined plots...")
        m = EfficientAD(device=str(device), img_size=args.img_size)
        m.load(ead_ckpt)
        all_records["EfficientAD"] = collect_predictions(m, test_loader, device)

    #  Combined plots if we have multiple models 
    if len(all_records) > 1:
        plot_combined_roc(
            all_records,
            save_path=os.path.join(args.results_dir, "roc_curves_all.png"),
        )
        plot_combined_score_dist(
            all_records,
            save_path=os.path.join(args.results_dir, "score_distribution_all.png"),
        )
